"""Support for Gree Hybrid switch entities."""
...
from .coordinator import DeviceDataUpdateCoordinator, GreeCloudConfigEntry, is_hwhp_device
...
class GreeCloudSwitchEntityDescription(SwitchEntityDescription):
    """Describes a Gree Hybrid switch entity."""

    get_value_fn: Callable[[GreeDevice], bool]
    set_value_fn: Callable[[GreeDevice, bool], None]


def _set_light(device: GreeDevice, value: bool) -> None:
    """Typed helper to set device light property."""
    device.light = value


def _set_quiet(device: GreeDevice, value: bool) -> None:
    """Typed helper to set device quiet property."""
    device.quiet = value


def _set_fresh_air(device: GreeDevice, value: bool) -> None:
    """Typed helper to set device fresh_air property."""
    device.fresh_air = value


def _set_xfan(device: GreeDevice, value: bool) -> None:
    """Typed helper to set device xfan property."""
    device.xfan = value


def _set_anion(device: GreeDevice, value: bool) -> None:
...
async def async_setup_entry(
    ...
    """Set up the Gree Hybrid HVAC device from a config entry."""
    ...
    entry.async_on_unload(async_dispatcher_connect(hass, DISPATCH_DEVICE_DISCOVERED, init_device))


class GreeCloudSwitch(GreeCloudEntity, SwitchEntity):
    """Generic Gree Hybrid switch entity."""
    ...
    def __init__(
        ...
        """Initialize the Gree Hybrid device."""
