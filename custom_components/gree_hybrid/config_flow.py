"""Config flow for Gree Hybrid integration."""
...
class GreeCloudConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    """Handle a config flow for Gree Hybrid."""
        ...
        if user_input is not None:
            ...
            try:
                ...
                return self.async_create_entry(
                    title=f"Gree Hybrid ({user_input[CONF_USERNAME]})",
                    data=user_input,
                )
                ...
                _LOGGER.exception("Unexpected error during Gree Hybrid login: %s", err)
