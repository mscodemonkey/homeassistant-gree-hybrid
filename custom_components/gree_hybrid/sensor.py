"""Support for Gree Hybrid sensor entities."""
...
class GreeCloudSensorEntityDescription(SensorEntityDescription):
    """Describes a Gree Hybrid sensor entity."""

    value_fn: Callable[[GreeDevice], float | None]
    exists_fn: Callable[[GreeDevice], bool]


def _energy_total(device: GreeDevice) -> float | None:
...
def _has_energy_total(device: GreeDevice) -> bool:
...
def _compressor_frequency(device: GreeDevice) -> float | None:
    """Return compressor frequency in Hz."""
    return device.raw_properties.get(PROP_COMPRESSOR_FREQ)


def _has_compressor_frequency(device: GreeDevice) -> bool:
...
def _humidity(device: GreeDevice) -> float | None:
    """Return relative humidity in percent."""
    return device.raw_properties.get(PROP_HUMIDITY)


def _has_humidity(device: GreeDevice) -> bool:
...
async def async_setup_entry(
    ...
    """Set up the Gree Hybrid sensors from a config entry."""
...
class GreeCloudSensor(GreeCloudEntity, SensorEntity):
    """Generic Gree Hybrid sensor entity."""
    ...
    def __init__(
        ...
        """Initialize the Gree Hybrid sensor."""
